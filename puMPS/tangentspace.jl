...
function tangent_space_MPO!(op_effs::Vector{Array{T,6}}, M::puMPState{T}, op::MPO_open{T}, ks::Vector{<:Real}) where {T}
    ...
    op_1 = op[1]; op_n = op[2]
    GC.enable(false)
    #println("op_1", size(op_1))
    #println("op_n", size(op_n))
    #println("blk_r", size(blk_r))
    ...
    @tensor part[V1b,Pb,V2b, V1t,Pt,V2t] = ((op_1[m3,pt,m1,Pb] * op_n[m1,Pt,m2,pb]) * A[vt,pt,V1t]) * (conj(A[V2b,pb,vb]) * blk_r[V2t,m2,vb, vt,m3,V1b])
    GC.enable(true)
    GC.gc(false)
    for j in 1:length(ks)
...
function tangent_space_MPO_boundary!(op_effs::Vector{Array{T,6}}, M::puMPState{T}, op_b::MPO_open{T},
    ...
    #println("blk ",size(blk))
    #sleep(10)
    op_b_n = op_term_MPO_boundary(M, op_b, 2)
    #println("op_b_n ",size(op_b_n))
    #sleep(10)
    #println("op_b_1 ",size(op_b_1))
    #println("A ",size(A)) 
    GC.enable(false)
    if length(op_b_n) == 0
        ...
        @tensor part[V1b,Pb,V2b, V1t,Pt,V2t] = (A[vt,pt,V1t]*blk[V2t,m2,vb, vt,m3,V1b])*(conj(A[V2b,pb,vb])*(op_b_1[m3,pt,m1,Pb]*op_b_n[m1,Pt,m2,pb]))
    end
    ...
    for n in 3:N-1
        ...
        GC.enable(true)
        GC.gc(false)
        #println("blkL_cgap ",size(blkL_cgap))
        #println("blkR_gap ",size(blkR_gap))
        GC.enable(false)
    ...
    if length(op_b_n) == 0
        ...
        @tensor part[V1b,Pb,V2b, V1t,Pt,V2t] = ((op_b_1[m3,pt,m1,Pb] * op_b_n[m2,Pt,m3,pb]) * conj(A[vb,pb,V1b])) * (A[V2t,pt,vt] * blk[vt,m1,V2b, V1t,m2,vb])
    end
    GC.enable(true)
    GC.gc(false)
    for j in 1:length(ks)
...
function tangent_space_MPO(M::puMPState{T}, op::Union{MPO_PBC_uniform_split{T}, MPO_PBC_split{T}}, ks::Vector{<:Real}) where {T}
    ...
    bTMs = nothing #free up this space
